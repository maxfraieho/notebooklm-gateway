...
> **DEPRECATED:** Цей сайт документує gh-aw (GitHub Agentic Workflows), який більше **не є canonical execution layer** проєкту Garden Seedling.
> Canonical runtime: **Mastra** (agent interpreter) + **Inngest** (orchestration) + **MinIO** (source of truth).
> Див. [GH_AW_DEPRECATION_NOTICE](/gh-aw/../deprecated/GH_AW_DEPRECATION_NOTICE/) та `docs/architecture/` для поточної архітектури.
